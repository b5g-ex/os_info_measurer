...
```elixir
# 計測を開始（tmp ディレクトリに csv_test_(free|proc_stat).csv として100ms間隔で記録）
OsInfoMeasurer.start("tmp", "csv_test", 100)
...
case OsInfoMeasurer.start("tmp", "test", 500) do
  ...
  {:error, :directory_not_found} ->
    Logger.error("ディレクトリが見つかりません")

  {:error, :invalid_interval} ->
    Logger.error("計測間隔は0より大きい値を指定してください")

  {:error, reason} ->
    Logger.error("エラー: #{inspect(reason)}")
end
```
...
※バイナリは `mix compile` によって、`priv/measurer` が生成されます。
