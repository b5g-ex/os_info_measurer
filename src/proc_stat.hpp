...
public:
  ...
  void dump_to_csv(std::filesystem::path data_directory_path) override
  {
    auto csv_file_path = data_directory_path / "proc_stat.csv";
    std::ofstream csv_file_stream(csv_file_path.string());

    // header, see man 5 proc, /proc/stat
    csv_file_stream
        << "measurement_time[ms]"s
        << ","s
    ...
    for (auto m : Measurer::measurements)
    {
      const auto measurement_time =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              ...
              .count();
      csv_file_stream << std::to_string(measurement_time) << ","s << m.data()
                      << "\n"s;
    }
